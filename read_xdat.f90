...
SUBROUTINE read_xdat(ntimesteps,nions,totatoms,acell,volume,natom,xyz,atype,sim_type)
  ...
  REAL, DIMENSION(:), INTENT(OUT) :: volume(ntimesteps)
  REAL, DIMENSION(:,:,:), INTENT(OUT) :: acell(ntimesteps,3,3), xyz(ntimesteps,totatoms,3)
  REAL :: scale
  ...
  ! Check to see if unwrapped file exists
  ! Only specific to VACF and MSD
  ! TRUE == EXISTS
  INQUIRE(FILE='unwrapped',EXIST=file_exists)

  IF (file_exists) THEN
    PRINT*, 'UNWRAPPED FILE EXISTS'
  ELSE 
    CALL EXECUTE_COMMAND_LINE('unwrap_cleanXDAT')
  END IF

  OPEN(UNIT=10,FILE='unwrapped',STATUS='OLD',ERR=200)
